...
func (service *learningStore) CreateUser(username string, password []byte, salt []byte, firstName string, lastName string) (int64, error) {
...
func (service *learningStore) GetUserByUsername(username string) (models.User, error) {
	...
	`, username).Scan(&user.ID, &user.Password, &user.Salt)
